...
function NewSectionTitle({section}: Props) {
  ...
  return (
    ...
    <div className='flex justify-between items-start p-6'>
        ...
        {
            !editMode &&
            <h3>{course.sections.find((s)=>s.id_section===section)?.title}</h3>
        }
        </div>
        {
            !editMode &&
    ...
    </div>
    {
        ...
        <CardFooter className='flex justify-end gap-2'>
            <Input value={course.sections.find((s)=>s.id_section===section)?.title} onInput={(e)=>updateCourse({
                ...course,
                sections:course.sections.map((s)=>s.id_section===section?{...s,title:(e.target as HTMLInputElement ).value}:s)
            })}></Input>
