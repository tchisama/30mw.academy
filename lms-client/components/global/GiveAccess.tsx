...
function GiveAccess({children,id,courses}: Props) {
  ...
  const [selectedCourse, setSelectedCourse] = React.useState<Course>({} as Course)
  ...
  const [price, setPrice] = React.useState(0)
    ...
    const giveAccess = ()=>{
      const alreadyHaveAccess = accesses?.find((access)=>access.id_course===selectedCourse._id)
    ...
    const removeAccess = (id_access:string)=>{
      axios.delete(server+"auth/remove-access/",{
        data:{
          id_access
        }
      }).then(()=>{
        ...
        setAccesses((p :any[] | null)=>{
          ...
          return(
            p?.filter((access)=>access._id!==id_access)
          )
          })
      })
    }
          ...
          <DialogDescription>
            <div className='mt-4 flex flex-col   gap-2'>
                ...
                <SelectContent className='flex-1'>
                  {
                    courses.map((course)=>{
                      return (
                        <SelectItem key={course._id} value={course._id as string}>{course.title}</SelectItem>
                      )
                    })
                  }
                </SelectContent>
              ...
              <div className='flex gap-2 items-center'>
                <Input value={price} onInput={(e)=>setPrice((e.target as HTMLInputElement).valueAsNumber)} placeholder="price you got" className='flex-1' type="number" />
                Dh
                <Button onClick={giveAccess}>
                  Give Access
                </Button>
              </div>
            </div>
            ...
            <div className='p-4  bg-slate-50 mt-2 rounded-xl border'>
              {
                ...
                (
                  ...
                  accesses.map((access:any)=>{
                    const title = courses.find((course)=>course._id===access.id_course)?.title
                    return (
                      title &&
                      <div className='mt-1 py-2 px-2 flex justify-between items-center bg-white text-right rounded-md border'>
                        <Button size={"icon"} variant={"outline"} onClick={()=>removeAccess(access._id)}><Trash size={16}/></Button>
