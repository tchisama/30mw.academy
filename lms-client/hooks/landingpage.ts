...
export const useLandingPage = create<StateType>()((set) => ({
  ...
  addSection: (section: SectionType, index?: number) => set((state) => {
    let newState = { ...state }
    if (index) {
  ...
  setSelectedSection: (section: SectionType | null) => set({ selectedSection: section }),
  updateSection: (section: SectionType) => set((state) => {
    let newState = { ...state }
